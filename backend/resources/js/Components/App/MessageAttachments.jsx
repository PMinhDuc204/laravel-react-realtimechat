...
const MessageAttachments = ({ attachments, attachmentsClick}) => {
  return (
    <>
      {attachments.length > 0 && (
        <div className="mt-2 flex flex-wrap justify-end gap-1">
          {attachments.map((attachment, ind) => (
            <div
              onClick={(ev) => attachmentsClick(attachments, ind)}
              key={attachment.id}
              className={
            ...
            >
              {!isAudio(attachment) && (
                <a
                  ...
                  download
                  href={attachment.url}
                  className="z-20 opacity-100 group-hover:opacity-100 transition-all w-8 h-8 flex items-center justify-center
